...
try:
    ...
    with col2:
        ...
        dias = st.select_slider(
            "How many days", [30, 60, 90, 120, 180, 360, 720, 1080, 1440, 1180, 1540, 1900])
        # define the indicators in the page
        indicators = st.multiselect("Indicators", ["sma_21", "sma_50", "sma_200", "ema_21",
                                    "ema_50", "ema_200", "macd_21", "macd_50", "macd_200", "rsi_21", "rsi_50", "rsi_200"])
    ...
    with col1:
        ...
        sma_21 = ta.sma(share_ibov_norm, length=21)
        # Calcula a média móvel simples
        sma_50 = ta.sma(share_ibov_norm, length=50)
        # Calcula a média móvel simples
        sma_200 = ta.sma(share_ibov_norm, length=200)
        # Calcula a média móvel exponencial
        ema_21 = ta.ema(share_ibov_norm, length=21)
        # Calcula a média móvel exponencial
        ema_50 = ta.ema(share_ibov_norm, length=50)
        # Calcula a média móvel exponencial
        ema_200 = ta.ema(share_ibov_norm, length=200)
        ...
        rsi_14 = ta.rsi(share_ibov_norm, length=14)
        # Calcula o Índice de Força Relativa
        rsi_21 = ta.rsi(share_ibov_norm, length=21)
        # Calcula o Índice de Força Relativa
        rsi_50 = ta.rsi(share_ibov_norm, length=50)
        ...
        with col22:
            ...
            st.write(
                f" - The return of **{share}** was: **{retorno_ibov:.1%}** in **{dias}** days")
            st.write(
                f" - The return of **{share_to_buy}** was: **{retorno_share:.1%}** in **{dias}** days")
            st.write(
                f" - The correlation between **{share}** and **{share_to_buy}** is: {share_ibov.corr(df_ibov):.1%}")

            if retorno_share > retorno_ibov:
                st.write(
                    f" - The **{share_to_buy}** was better then the **{share}** in the period of **{dias}** days")
            else:
                st.write(
                    f" - The **{share}** was better then the **{share_to_buy}** in the period of **{dias}** days")
        ...
        with col33:
            ...
            st.write(
                f" - The **{share_to_buy}** price was **{price_store.iloc[0]:,.2f}** in **{date_store}**")
            try:
                if sma_200[-1] < sma_50[-1]:
                    st.write(
                        f" - The Ticker **{share_to_buy}** trend is **upward** according to the moving average")
                else:
                    st.write(
                        f" - The Ticker **{share_to_buy}** trend is **down** according to the moving average")
            except:
                pass
